...
function updateFile() {
    ...
    //fr.readAsText(file);
    fr.onload = function () { getFileType(fr.result); };
}
function arrLenEqual(a, b) {
    return b.every((val, index) => val === a[index]);
}
function getFileType(data) {
    let dataView = new Uint8Array(data);
    if (arrLenEqual(dataView, new Uint8Array([0x49, 0x49, 0x2a, 0x00])))
        fileType = "tiff";
    else if (arrLenEqual(dataView, new Uint8Array([0x00, 0x00, 0x01, 0x00, 0x01, 0x00])))
        fileType = "ico";
    else if (arrLenEqual(dataView, new Uint8Array([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a])))
        fileType = "png";
    else if (arrLenEqual(dataView, new Uint8Array([0x42, 0x4d])))
        fileType = "bmp";
    else
        fileType = "Unknown";
...
function toTiff(data) {
}
function toIco(data) {
}
function fromPNG(data) {
            ...
            if (bpp == 24) {
                for (let i = 0; i < height; i++) {
                    for (let j = 0; j < width; j++) {
                        ...
                        console.log(JSON.parse(JSON.stringify(decodedData[i])));
                    ...
                    console.log('b', decodedData[0][0]);
                }
                console.log('c', decodedData[0][0]);
            }
            console.log('d', decodedData[0][0]);
            break;
        case 0x1:
            break;
        default:
